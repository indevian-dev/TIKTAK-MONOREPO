...
export function StaffMailConfigurationWidget() {
  ...
  const fetchConfiguration = async () => {
    try {
      const response = await fetch('/api/staff/mail/config')
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const response = await fetch('/api/staff/mail/test', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        ...
        <form onSubmit={handleSaveConfiguration} className="space-y-6">
          ...
          <div className="flex space-x-4">
            <button
              type="submit"
              disabled={saving}
              className="px-4 py-2 bg-brandPrimary text-white rounded-md hover:bg-brandPrimary/90 focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
