...
export function StaffCardDetailsModalWidget({ 
  ...
  useEffect(() => {
    if (selectedCard) {
      ...
      if (selectedCard.published_data?.category?.id) {
        allCategoryIds.push(selectedCard.published_data.category.id);
      }
  ...
  const fetchOptionsForCategories = async (categoryIds: number[]): Promise<void> => {
  ...
  const findOptionById = (optionId: number): { option: Option; group: OptionGroup } | null => {
              ...
              {value.length > 0 ? (
                value.map((img, idx) => (
                  <div key={idx} className="relative">
                    <Image
                      src={`${Bun.env.NEXT_PUBLIC_S3_PREFIX}/cards/${selectedCard.storage_prefix}/${img}`}
                      alt={`Image ${idx + 1}`}
                      width={80}
                      height={80}
