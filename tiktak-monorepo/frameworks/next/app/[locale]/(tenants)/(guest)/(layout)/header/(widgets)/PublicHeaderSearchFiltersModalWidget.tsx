...
export function PublicHeaderSearchFiltersModalWidget({
  ...
  return (
    <div className="fixed inset-0 bg-brandPrimaryDarkBg/50 flex items-start justify-center z-[200] backdrop-blur-sm pt-4 px-4">
      <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200">
        ...
        <div className="flex items-center justify-between p-4 border-b border-light">
          <div className="flex items-center gap-2">
            <PiMagnifyingGlassLight className="text-xl text-brandPrimary" />
            <PiFunnelLight className="text-xl text-brandPrimary" />
            <PiSortAscendingLight className="text-xl text-brandPrimary" />
            <h3 className="text-lg font-bold text-dark ml-2">
              {t('search_and_filters')}
            </h3>
          </div>
          <button
            onClick={onClose}
            className="p-2 hover:bg-brandPrimaryLightBg rounded-full transition-colors"
            aria-label={t('close')}
          >
            <PiXLight className="text-2xl text-dark" />
          </button>
        </div>
                ...
                <button
                  ...
                  className={`
                    ...
                    ${localSort === option.value
                      ? 'bg-brandPrimary text-white border-brand'
        ...
        {/* Actions */}
        <div className="flex gap-3 p-4 border-t border-light bg-brandPrimaryLightBg/30">
          <button
            type="button"
            onClick={handleClear}
            className="flex-1 px-4 py-3 bg-brandPrimaryDarkBg hover:bg-brandPrimaryDarkBg/80 text-white font-medium rounded-lg transition-colors"
          >
            {t('clear_all')}
          </button>
          <button
            type="button"
            onClick={handleApply}
            className="flex-1 px-4 py-3 bg-brandPrimary hover:bg-brandPrimary/80 text-white font-medium rounded-lg transition-colors"
          >
            {t('apply')}
          </button>
        </div>
