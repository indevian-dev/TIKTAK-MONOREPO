...
export function PublicSearchWidget({ hideOnCardFilters = false }: PublicSearchWidgetProps) {
    ...
    useEffect(() => {
        const fetchCities = async () => {
            try {
                const response = await apiFetchHelper({ method: 'GET', url: '/api/cities', params: {}, body: {} });
    ...
    return (
        <div className="ml-4 flex items-center relative">
            ...
            <div className="md:hidden">
                ...
                >
                    {!isSearchVisible ? (
                        <PiMagnifyingGlassLight className='text-dark text-2xl' />
                    ) : (
                        <PiXCircleLight className='text-dark text-2xl' />
                    )}
                </button>
            </div>
            ...
            {isSearchVisible && (
                <div className="md:hidden fixed top-0 left-0 right-0 bg-brandPrimaryLightBg/60 z-30">
                    <div className="flex items-center gap-2 px-4 py-3">
                        <div className="flex-1 relative">
                            <input
                                className='w-full px-3 py-2 pr-10 bg-white rounded text-sm text-dark placeholder:text-gray-500'
                                value={searchQuery}
                        ...
                        <select
                            className="w-24 px-2 py-2 rounded text-sm bg-white text-dark"
                            value={selectedCity}
                        ...
                        <button className="" onClick={() => setIsSearchVisible(false)}>
                            <PiXCircleLight className='text-dark text-xl' />
                        </button>
                    </div>
            ...
            <div className="hidden md:flex items-center gap-4 relative">
                <div className="relative">
                    <input
                        className='w-80 px-5 py-2 pr-12 bg-brandPrimaryLightBg/60 rounded text-lg text-dark placeholder:text-dark placeholder-opacity-100'
                        value={searchQuery}
                        onChange={handleSearchChange}
                        placeholder={t('search_cards_placeholder')}
                    />
                    ...
                    {searchQuery && (
                        <button
                            onClick={clearSearch}
                            className="absolute right-3 top-1/2 transform -translate-y-1/2 text-dark hover:text-gray-600 transition-colors"
                        >
                            <PiXCircleLight className="text-lg" />
                        </button>
                    )}
                ...
                <select
                    className="w-40 px-2 py-2.5  rounded text-lg bg-brandPrimaryLightBg/60 text-dark"
                    value={selectedCity}
                    onChange={(e) => setSelectedCity(e.target.value)}
                >
