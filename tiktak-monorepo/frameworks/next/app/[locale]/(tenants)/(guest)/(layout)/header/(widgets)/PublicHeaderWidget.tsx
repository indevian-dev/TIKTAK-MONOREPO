...
import { PublicHeaderCategoriesMenuWidget }
  from '@/app/[locale]/(tenants)/(guest)/categories/(widgets)/PublicHeaderCategoriesMenuWidget';
import { PublicAltHeaderWidget }
  from '@/app/[locale]/(tenants)/(guest)/(layout)/header/(widgets)/PublicAltHeaderWidget';
...
import { PublicHeaderNavDefault }
  from '@/app/[locale]/(tenants)/(guest)/(layout)/header/(widgets)/PublicHeaderNavDefault';
import { PublicHeaderNavCard }
  from '@/app/[locale]/(tenants)/(guest)/(layout)/header/(widgets)/PublicHeaderNavCard';
import { PublicHeaderNavCategory }
  from '@/app/[locale]/(tenants)/(guest)/(layout)/header/(widgets)/PublicHeaderNavCategory';
import { PublicHeaderNavStore }
  from '@/app/[locale]/(tenants)/(guest)/(layout)/header/(widgets)/PublicHeaderNavStore';
import { Link }
  from '@/i18n/routing';
import Image
  from 'next/image';
  ...
  return (
    <>
      <header className="z-100 sticky top-0 bg-white w-full shadow-2xl shadow-dark/10">
        ...
        <nav className="z-4 text-dark relative m-auto max-w-7xl items-center justify-between px-4 py-2">
          <div className="flex items-center justify-between w-full gap-2">
            <Link href="/" className=' flex items-center gap-2 bg-brandPrimaryDarkBg sm:bg-transparent rounded p-2'>
          ...
          <div className={`overflow-y-scroll overflow-x-hidden transform ${catalogueVisible ? 'fixed' : 'hidden'} transition duration-500 ease-in-out right-0 left-0 text-dark h-screen bg-white z-0`}>
            <ul className="pt-4 pb-40 text-dark">
