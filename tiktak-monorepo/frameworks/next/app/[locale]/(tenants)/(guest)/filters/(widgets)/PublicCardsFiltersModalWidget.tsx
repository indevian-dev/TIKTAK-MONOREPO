...
import type { Category } from '@/app/[locale]/(tenants)/(guest)/categories/PublicCategoriesService';
...
export function PublicCardsFiltersModalWidget({
    ...
    useEffect(() => {
        const fetchFiltersIfNeeded = async () => {
            ...
            if (userFilters.categories && userFilters.categories.length > 0) {
                try {
                    const { filters, error } = await getCategoryFilters(userFilters.categories);
    ...
    const handleCategoryChange = useCallback(async (categoryId: string | number, level: number) => {
        ...
        if (!categoryId) {
            ...
            try {
                const { categories: subCategories, error } = await getSubCategories(numericCategoryId);
        ...
        <>
            ...
            <div className={`flex justify-start w-full h-full ${className}`}>
                <button
                    ...
                    className={`
                        flex items-center transition-all duration-200 bg-white text-dark  w-full justify-center
                    `}
            ...
            {/* Filters Modal */}
            {isModalOpen && (
                <div className="fixed inset-0 bg-brandPrimaryLightBg bg-opacity-70 flex items-center justify-center z-100 bg-blure backdrop-blur-sm">
                    <div className="bg-white/80 rounded-lg w-full mx-3 md:mx-6 lg:mx-12 xl:mx-16 max-w-7xl min-w-[80hv] max-h-[80vh] overflow-hidden bg-opacity-50 ">
                        <div className="flex flex-col h-full max-h-[80vh]">
                            ...
                            <div className="flex items-center justify-between p-3 border-b bg-white">
                                <h3 className="text-xl font-bold text-dark">{t('filters')}</h3>
                                <button
                                    onClick={() => setIsModalOpen(false)}
                                    className="text-5xl font-balck text-brandPrimary"
                            ...
                            <div className="flex-1 overflow-y-auto p-6">
                                <div className="grid grid-cols-12 gap-6 h-full">
                                    ...
                                    <div className="space-y-4 col-span-12">
                                        <h4 className="font-bold text-lg text-dark border-b pb-2">{t('selected_filters')}</h4>
                                            ...
                                            {selectedFilters.length === 0 ? (
                                                ...
                                                <div className="flex flex-wrap gap-2 pb-2">
                                                    {selectedFilters.map(filter => (
                                                        <div key={filter.id} className="flex items-center bg-brandPrimaryLightBg rounded-full px-3 py-1 whitespace-nowrap border">
                                                            <span className="text-sm text-dark">{filter.label}</span>
                                                            <button
                                    ...
                                    <div className="space-y-4 col-span-12 md:col-span-4">
                                        <h4 className="font-bold text-lg text-dark border-b pb-2">{t('categories')}</h4>
                                    ...
                                    <div className="space-y-4 col-span-12 md:col-span-4">
                                        <h4 className="font-bold text-lg text-dark border-b pb-2">{t('price_filters')}</h4>
                                        <div className="space-y-4 max-h-96 overflow-y-auto">
                                    ...
                                    <div className="space-y-4 col-span-12 md:col-span-4">
                                        <h4 className="font-bold text-lg text-dark border-b pb-2">{t('sort_options')}</h4>
                                        <div className="space-y-4 max-h-96 overflow-y-auto">
                                            {/* Main Sort Dropdown */}
                                            <div className="space-y-2">
                                            ...
                                            {(selectedSort.includes('price') || selectedSort.includes('title') || selectedSort.includes('rating')) && (
                                                <div className="space-y-2">
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex items-center gap-2 px-4 py-2 bg-brandPrimaryLightBg hover:bg-semilight rounded-md text-sm font-medium transition-colors duration-200 w-full"
                                                    >
                                            ...
                                            {selectedSort !== 'newest' && (
                                                <div className="pt-2 border-t">
                                                    <button
                                                        ...
                                                        className="w-full px-4 py-2 text-sm font-medium text-brandPrimary border border-brand rounded-md hover:bg-brandPrimary hover:text-white transition-colors duration-200"
                                                    >
                                                        {t('reset_to_default')}
                                                    </button>
                                                </div>
                            ...
                            <div className="flex gap-3 justify-end p-3 border-t bg-white pl-12">
                                <button
                                    ...
                                    className="px-6 py-2 bg-brandPrimaryDarkBg hover:bg-brandPrimaryDarkBg/80 text-white font-medium rounded-lg transition-colors duration-200 border border-dark"
                                >
                                    {t('clear_all')}
                                </button>
                                <button
                                    ...
                                    className="px-6 py-2 bg-brandPrimary hover:bg-brandPrimary/80 text-white font-medium rounded-lg transition-colors duration-200 border border-brand"
                                >
                                    {t('apply_filters')}
                                </button>
                            </div>
