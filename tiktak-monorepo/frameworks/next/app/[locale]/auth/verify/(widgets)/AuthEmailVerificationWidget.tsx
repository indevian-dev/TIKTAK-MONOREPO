 ...
 * or the provided mode prop. The contact field is editable so users can update their email if needed.
 * 
...
export function AuthVerificationWidget({
  ...
  const { t } = loadClientSideCoLocatedTranslations('AuthEmailVerificationWidget');
  ...
  const [contact, setContact] = useState(phoneFromQuery || emailFromQuery || initialContact || '');
  ...
  }, [emailFromQuery, phoneFromQuery, modeFromUrl]);

  // Auto-fill contact from user profile if available
  useEffect(() => {
    if (!user || contact) return;

    if (mode === 'email' && user.email) setContact(user.email);
  ...
  async function handleSendCode() {
    try {
      ...
      const response = await apiFetchHelper({
        url: '/api/auth/verify/email/generate',
        method: 'POST',
      ...
      if (result.alreadyVerified) {
        toast.info(result.message || t('already_verified').replace('{mode}', mode === 'email' ? t('email') : t('phone')));
      } else {
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    try {
      ...
      const response = await apiFetchHelper({
        url: '/api/auth/verify/email/validate',
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      });
      ...
      toast.success(result.message || (texts.successMessage || t('verification.success_message')).replace('{mode}', mode === 'email' ? t('email') : t('phone')));
