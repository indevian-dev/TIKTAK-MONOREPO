...
import type { ApiError } from '@tiktak/shared/types/api/ApiResponse.types';
import type { AuthContextPayload } from '@tiktak/shared/types/auth/Auth.types';
...
async function executeRequest({
  ...
  try {
    ...
    // ═══ Auto-unwrap standardized API envelope ═══
    // { success: true, data: T } → response.data becomes T
    // Backward compatible: if no envelope, data passes through unchanged
...
export async function retryAfter2FA(originalRequest: FetchOptions): Promise<AxiosResponse> {
  ...
  return apiCall({ method, url, params, body, headers });
}
