...
export class UserRepository extends BaseRepository {
    ...
    async findByEmail(email: string, tx?: DbClientTypes) {
        const client = tx ?? this.db;
        const result = await client
            .select()
            .from(users)
    ...
    async list(options: UserListOptions, tx?: DbClientTypes) {
        const client = tx ?? this.db;
    ...
    async count(search?: string, searchType?: string, tx?: DbClientTypes) {
        const client = tx ?? this.db;
    ...
    async update(id: string, data: Partial<typeof users.$inferInsert>, tx?: DbClientTypes) {
        const client = tx ?? this.db;
        const result = await client
            .update(users)
