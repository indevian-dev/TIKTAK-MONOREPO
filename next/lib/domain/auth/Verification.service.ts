...
export class VerificationService extends BaseService {
    ...
    async validateOtp(params: { type: 'email' | 'phone', target: string, otp: string, accountId: string, userId: string }): Promise<AuthResult> {
        try {
            ...
            // 2. Update User
            const updateData: { updatedAt: Date; email?: string; emailIsVerified?: boolean; phone?: string; phoneIsVerified?: boolean } = {
                updatedAt: new Date(),
            };
            if (type === 'email') {
            ...
            const { SessionStore } = await import('@/lib/middleware/Store.Session.middleware');
