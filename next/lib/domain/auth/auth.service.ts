...
import { verifyPassword, validatePassword, hashPassword } from "@/lib/utils/security/passwordUtility";
import { cleanPhoneNumber, validateAzerbaijanPhone } from "@/lib/utils/formatting/phoneFormatterUtility";
import { generateSlimId } from "@/lib/utils/ids/slimUlidUtility";
import { SessionAuthenticator } from "@/lib/middleware/authenticators/SessionAuthenticator";
import { assignAccountScope, mapSkopeTypeToDomain, type SkopeType } from "@/lib/utils/path/skopeUtility";
import { v4 as uuidv4 } from "uuid";
import type { AuthContext } from "@/lib/domain/types";
import {
...
import { sendMail } from '@/lib/notifications/mail/mailService';
import { generateVerificationOtpEmail } from '@/lib/notifications/mail/mailGenerator';
import { sendOtpSmsPlus } from '@/lib/notifications/sms/smsService';
import s3 from "@/lib/integrations/aws/awsClient";
...
export class AuthService extends BaseService {
    ...
    async verifyCode(params: {
        accountId: string;
        code: string;
        operation: 'verification' | 'email_change' | 'phone_change' | '2fa' | 'password_reset';
        email?: string;
        phone?: string;
    }): Promise<AuthResult> {
        try {
            const { accountId, code, operation, email, phone } = params;

            // 1. Map operation to OTP type
            let otpType: OtpType;
            if (operation === 'verification') {
                otpType = email ? 'email_verification' : 'phone_verification';
            } else if (operation === 'email_change') {
                otpType = 'email_change' as OtpType;
            } else if (operation === 'phone_change') {
                otpType = 'phone_change' as OtpType;
            } else {
                otpType = operation as OtpType;
            }
            ...
            if (!validOtp) {
                return { success: false, error: "Invalid or expired verification code", status: 400 };
            }

            // 3. Handle specific operations
            if (operation === 'verification') {
                ...
                await this.repository.updateUser(account.userId, {
                    [email ? 'emailIsVerified' : 'phoneIsVerified']: true
                });
            } else if (operation === 'email_change' && email) {
                const account = await this.repository.findAccountById(accountId);
                if (!account) return { success: false, error: "Account not found", status: 404 };
            ...
            } else if (operation === 'phone_change' && phone) {
                const account = await this.repository.findAccountById(accountId);
                if (!account) return { success: false, error: "Account not found", status: 404 };
