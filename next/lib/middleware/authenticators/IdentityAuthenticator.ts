...
import { users, accounts, userCredentials, workspaces, workspaceAccesses, workspaceRoles } from "@/lib/database/schema";
import { eq, or, and, sql } from "drizzle-orm";
import {
  validatePassword,
  hashPassword,
  verifyPassword,
} from "@/lib/utils/security/passwordUtility";
...
async function getRedisClient(): Promise<RedisClient | null> {
  if (!redisClient && CACHE_CONFIG.useRedis) {
    try {
      const redis = await import("@/lib/integrations/upstash/cacheRedis");
