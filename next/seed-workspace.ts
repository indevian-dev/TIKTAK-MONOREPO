...
async function main() {
    ...
    await db.execute(sql`
    UPDATE cards
    SET store_id = ${WORKSPACE_ID}, workspace_id = ${WORKSPACE_ID}, updated_at = NOW()
    WHERE id LIKE 'SEED_CARD_%' OR id = 'FULL_TEST_CARD_001'
  `);
    const cnt = await db.execute(sql`SELECT COUNT(*) as c FROM cards WHERE store_id = ${WORKSPACE_ID}`);
