        ...
        <section className={`relative m-auto max-w-screen-xl px-4 ${theme}`}>
            ...
            <div className='flex gap-2 py-2 w-full justify-between bg-white'>
                <div className='flex gap-2'>
                    <button onClick={() => switchLanguage('en')} className='p-2 hover:text-brandPrimary'>EN</button>
                    <button onClick={() => switchLanguage('es')} className='p-2 hover:text-brandPrimary'>ES</button>
                </div>
                <button onClick={toggleTheme} className='p-2 hover:text-brandPrimary flex gap-2 items-center'>
            ...
            <div className="mb-6">
                {topMenuItems.map((item) => (
                    <div key={item.id} className='w-full py-2' onClick={() => handleItemClick(item.slug)}>
                        <h2 className='text-lg font-bold cursor-pointer p-2 text-dark hover:text-brandPrimary'>
                            {item.title}
                        </h2>
                    </div>
                ))}
            ...
            <div className="mt-6 mb-24 grid grid-cols-1 lg:grid-cols-2 gap-4">
                {Object.entries(bottomMenuItems).map(([key, section]) => (
                    <div key={key} className="mb-6 ">
                        <h3 className="text-md font-bold text-dark mb-3">{section.title}</h3>
                        {section.items.map((item) => (
                            <div key={item.id} className='w-full py-1' onClick={() => handleItemClick(item.slug)}>
                                <h2 className='text-md cursor-pointer p-2 text-dark hover:text-brandPrimary'>
                                    {item.title}
                                </h2>
                            </div>
                        ))}
