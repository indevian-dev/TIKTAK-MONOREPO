...
interface PublicAddOptionsToCardWidgetProps {
  ...
  onOptionsChange?: (options: Record<number, FilterOption[]>) => void;
}
...
const PublicAddOptionsToCardWidget = ({ 
  ...
  useEffect(() => {
    async function fetchCategories() {
      try {
        ...
        const buildHierarchy = (arr: Category[], parentId: number | null = null): Category[] => {
          return arr
  ...
  const handleCategoryChange = (categoryId: string, level: number) => {
    const newPath = [...selectedPath.slice(0, level), parseInt(categoryId)];
