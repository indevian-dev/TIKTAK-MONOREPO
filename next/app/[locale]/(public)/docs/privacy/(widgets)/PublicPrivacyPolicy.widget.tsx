...
export function PublicPrivacyWidget() {
    const [privacy, setPrivacy] = useState<PageData>({});
    const locale = useLocale();

    useEffect(() => {
        fetchPage('PRIVACY', locale)
            .then(data => { if (data) setPrivacy(data); })
            .catch(err => ConsoleLogger.error('Error fetching privacy page:', err));
    }, [locale]);
        ...
        <SectionPrimitive variant='centered'>
            <div className='container max-w-screen-xl mx-auto p-4 md:p-6 lg:p-8'>
                <div className="w-full flex flex-wrap justify-center shadow-md rounded-md">
                    <article className='w-full text-black dark:text-white p-5 prose-sm' dangerouslySetInnerHTML={{ __html: privacy.content || '' }}>
                    </article>
                </div>
            </div>
        </SectionPrimitive>
