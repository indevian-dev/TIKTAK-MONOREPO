...
import { ProviderMessageBubbleWidget }
  from '@/app/[locale]/workspaces/provider/[workspaceId]/conversations/(widgets)/ProviderMessageBubbleWidget';
import { ProviderConversationChatInputWidget }
  from '@/app/[locale]/workspaces/provider/[workspaceId]/conversations/(widgets)/ProviderConversationChatInputWidget';
import { ProviderConversationTypingIndicatorWidget }
  from '@/app/[locale]/workspaces/provider/[workspaceId]/conversations/(widgets)/ProviderConversationTypingIndicatorWidget';
...
export function ProviderConversationWidget({ conversationId }: ProviderConversationWidgetProps) {
  ...
  const {
    ...
    sendTypingIndicator,
    loadMessages
  } = useChat(`${conversationId}`, currentAccount?.id?.toString() || null) as {
  ...
  const loadConversation = async () => {
    try {
      ...
      const [convResponse] = await Promise.all([
        fetch(`/api/provider/conversations/${conversationId}`),
        loadMessages()
      ]);
        ...
        {messages.length === 0 ? (
          ...
          messages.map((message) => (
            <ProviderMessageBubbleWidget
              key={message.id}
              message={message}
              isOwnMessage={message.senderId === currentAccount?.id}
              senderName={message.senderId !== currentAccount?.id 
