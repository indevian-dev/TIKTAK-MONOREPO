...
export function ProviderOptionsSelectorWidget({ 
  ...
  const handleOptionChange = (filterId: string | number, optionValue: string | number, type: string) => {
    ...
    const filterIdNum = typeof filterId === 'string' ? parseInt(filterId) : filterId;
    const optionValueNum = typeof optionValue === 'string' ? parseInt(optionValue) : optionValue;
    
    if (!optionValue || optionValue === '') {
      setSelectedOptions(prevOptions => {
        const newOptions = prevOptions.filter(option => option.filter_id !== filterIdNum);
    ...
    setSelectedOptions(prevOptions => {
      ...
      const filteredOptions = currentOptions.filter(option => option.filter_id !== filterIdNum);
      ...
      const updatedOption = {
        ...
        filter_option_id: type === 'DYNAMIC' ? null : optionValueNum,
  ...
  const getCurrentValue = (filterId: string | number): string => {
    const filterIdNum = typeof filterId === 'string' ? parseInt(filterId) : filterId;
    const option = selectedOptions.find(opt => opt.filter_id === filterIdNum);
    if (!option) return '';
