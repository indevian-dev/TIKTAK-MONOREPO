    ...
    <div className={`relative group ${className}`}>
      <div className="flex gap-2 items-center relative hover:cursor-pointer justify-end">
        ...
        <div className="relative">
          ...
          {unreadCount > 0 && (
            <span className="absolute -top-2 -right-2 bg-brandPrimary text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold">
      ...
      <div className="absolute top-full right-0 mt-2 bg-white shadow-lg rounded-md p-4 hidden group-hover:block w-80 z-50">
        <div className="flex items-center justify-between mb-3">
          ...
          {unreadCount > 0 && (
            <span className="bg-brandPrimary text-white text-xs px-2 py-1 rounded-full">
              {unreadCount} new
            </span>
          )}
        </div>
              ...
              <li key={notification.id} className="border-b border-gray-100 pb-2 last:border-b-0">
                <div className="flex items-start gap-2">
                  {!notification.mark_as_read && (
                    <span className="w-2 h-2 bg-brandPrimary rounded-full mt-2 shrink-0"></span>
                  )}
        ...
        <Link
          ...
          className="block w-full bg-brandPrimary text-white px-4 py-2 rounded-md text-center text-sm font-medium hover:bg-red-600 transition-colors mt-4"
        >
