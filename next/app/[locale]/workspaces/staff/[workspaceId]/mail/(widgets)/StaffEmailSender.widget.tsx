'use client'
import { apiPost } from '@/lib/utils/ApiRequest.util';
...
export function StaffEmailSenderWidget() {
  ...
  const handleSendEmail = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiPost<EmailResult>('/api/staff/mail/send', formData)
      setResult(data)
        ...
        <form onSubmit={handleSendEmail} className="space-y-4">
          ...
          <div>
            <button
              type="submit"
              disabled={sending}
              className="flex items-center px-4 py-2 bg-app-bright-purple text-white rounded-md hover:bg-app-bright-purple/90 focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
