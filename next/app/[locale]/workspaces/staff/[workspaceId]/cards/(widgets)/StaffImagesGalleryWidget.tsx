...
export default function StaffImagesGalleryWidget({ images, storage_prefix = 'cards' }: { images: string[], storage_prefix?: string }) {
  ...
  return (
    <div className="flex flex-col items-center w-full md:max-w-[200px] lg:max-w-[300px] pl-4">
      ...
      {images.length > 1 && (
        <div>
          <button onClick={prevImage} className="mr-2 bg-brandPrimary text-dark hover:bg-brandPrimary/80 font-bold py-2 px-3 rounded-full">←</button>
          <button onClick={nextImage} className=' bg-brandPrimary text-dark hover:bg-brandPrimary/80 font-bold py-2 px-3 rounded-full'>→</button>
        </div>
      )}
    </div>
  );
